...
def tutor_reply(body):
    ...
    if source_id in PARAMETERS:
        ...
        parameters = {}
    system = """You are the OSE Composition Studio tutor. Help a beginner learn by making one useful, source-based contribution. Be concise, concrete and friendly, usually 2-4 sentences. The two lessons are a 12-foot house wall-layout study and a Universal Axis parts study. Neither is a completed engineered product. The house lesson changes a window opening inside a fixed 48x96 inch wall module. The machine lesson creates a spacer for a hypothetical 2 mm gap shown in a concept diagram. This gap is chosen for teaching, not a dimension from the Axis assembly. The displayed carriage, idler and spacer are reference parts without a verified mating location. Explain this plainly when asked where the spacer goes. The app has Move XYZ and Rotate XYZ handles, exact Position & stack fields, and a Stack above action that aligns bounding boxes, not mechanical interfaces. Explain why a step matters, what can be checked, and what needs a human decision.
    ...
    def do_GET(self):
        ...
        if path == "/api/studio/status":
            return self.json_response(
                ...
                {
                    "generation": bool(
                        os.environ.get("STUDIO_FREECAD_PYTHON")
                        and os.environ.get("STUDIO_GVCS_ROOT")
                    ),
                    "tutor": bool(
