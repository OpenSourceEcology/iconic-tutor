import {chapters, sources, audiencePrompts, bookMarkdown} from './book-content.js?v=1';
...
function saveStory() {const stored = save(storyKey,{...Object.fromEntries(new FormData(form)),audience:$('#story-audience').value}); $('#story-status').textContent = stored ? 'Your draft is saved in this browser. Nothing has been sent or published.' : 'Browser storage unavailable. Download your draft before leaving this page.';}
$('#story-audience').onchange = () => {updatePrompt(); saveStory();}; form.oninput = saveStory; updatePrompt();
form.onsubmit = event => {event.preventDefault(); const data = Object.fromEntries(new FormData(form)); if (Object.values(data).some(value => !value.trim())) {$('#story-status').textContent = 'Please complete each part of your story.'; return;} const text = `# My OSE story draft\n\nPersonal writing exercise — not an official OSE statement or funding offer.\nAudience: ${$('#story-audience').selectedOptions[0].text}\n\n## Possibility\n${data.possibility}\n\n## Example and source\n${data.example}\n\n## Uncertainty / evidence still needed\n${data.unknown}\n\n## Invitation\n${data.invitation}\n\nReading guide: https://opensourceecology.github.io/iconic-tutor/book.html\n`; download(text,'my-ose-story.md'); $('#story-status').textContent = 'Story draft downloaded. Review claims and sources before sharing; nothing was submitted.';};
